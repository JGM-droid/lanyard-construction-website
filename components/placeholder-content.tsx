  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        {eyebrow ? (
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-brand-accent">{eyebrow}</p>
        ) : null}
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          href="/contact"
          className="rounded-full bg-brand-accent px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#a8541f]"
        >
          Request a consultation
        </Link>
        <Link
          href="/services"
          className="rounded-full border border-stone-300 px-5 py-3 text-sm font-semibold text-brand-charcoal transition hover:border-brand-accent hover:text-brand-accent"
        >
          Explore services
        </Link>
      </div>
